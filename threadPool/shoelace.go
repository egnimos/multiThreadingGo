...
func findArea(inputChannel chan string) {
	for pointsStr := range inputChannel {
		var points []Point2D
		//assiging the line variable into thread
...
func main() {
	absPath, _ := filepath.Abs("/home/niteeshdubey/Documents/Workspace/GOLANG/multiThreadingGo/threadPool/")
	bs, _ := ioutil.ReadFile(filepath.Join(absPath, "polygons.txt"))
	text := string(bs)
